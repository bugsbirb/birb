...
class qotd(commands.Cog):
    ...
    async def FetchQuestion(self, Used, server: discord.Guild):
        ...
        if not isinstance(Used, list):
            Used = []
        Questions = (
            await self.client.db["Question Database"]
        ...
        Unusued = [
            q
            for q in Questions
            if isinstance(q, dict) and q.get("question") not in Used
        ]
    ...
    async def ProcesssQOTD(self, results):
        async with self.semaphore:
            try:
                ...
                UserGeneratedMO = (
                    " • This question is user generated and doesn't reflect on Birb as a whole."
                    if UserGenerated
                    else ""
                )
                embed.set_footer(
