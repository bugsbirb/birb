    ...
    async def InfractionTypes(
        ...
        try:
            ...
            if data.get("changegrouprole") and data.get("grouprole"):
                from core.integrations.roblox import UpdateMembership
...
async def DemotionSystem(
    ...
    if DemoSystemType == "multi":
        ...
        MemberRolesInHierarchy = [Role for Role in SortedRoles if Role in MemberRoles]
        OldRole = (
            max(MemberRolesInHierarchy, key=lambda Role: Role.position)
            if MemberRolesInHierarchy
            else None
        )
            ...
            if SkipRole and SkipRole in SortedRoles:
                ...
                await self.db["infractions"].update_one(
                    {"_id": DemotionData.get("_id")},
                    {
                        "$set": {
                            "new": SkipRole.id,
                            "previous": OldRole.id if OldRole else None,
                        }
                    },
                )
    ...
    if DemoSystemType == "single":
        ...
        if SkipTo:
            ...
            if not SkipRole:
                logger.warning("[Demotion] Invalid SkipTo role")
                return await self.db["infractions"].find_one(
                    {"_id": DemotionData.get("_id")}
                )

            if SkipRole not in SortedRoles:
                logger.warning("[Demotion] SkipTo not in hierarchy")
                return await self.db["infractions"].find_one(
                    {"_id": DemotionData.get("_id")}
                )

            if not guild.me.top_role > SkipRole:
                logger.warning("[Demotion] Bot cannot assign SkipRole (hierarchy)")
                return await self.db["infractions"].find_one(
                    {"_id": DemotionData.get("_id")}
                )
