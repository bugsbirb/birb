...
class quota(commands.Cog):
    ...
    async def ResetQuota(
        ...
        if (
            (quota == "Messages" and not Quota)
            or (quota == "Tickets" and not Tickets)
            or (quota == "Both" and not (Quota and Tickets))
        ):
            await ctx.send(embed=ModuleNotEnabled(), view=Support())
            return
    ...
    @Permissions("Admin")
    @ModuleIsEnabled("Staff Database")
    async def add(
        self,
