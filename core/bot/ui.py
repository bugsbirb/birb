...
class BasicPaginator(discord.ui.View):
    def __init__(
        self, author: discord.Member, messages: list = None, embeds: list = None
    ):
        super().__init__()
    ...
    @discord.ui.button(
        label="◀", style=discord.ButtonStyle.blurple, disabled=True, row=4
    )
    ...
    @discord.ui.button(
        label="1/1", style=discord.ButtonStyle.grey, disabled=True, row=4
    )
    async def page(self, _interaction: discord.Interaction, _button: discord.ui.Button):
        pass

    @discord.ui.button(
        label="▶", style=discord.ButtonStyle.blurple, disabled=False, row=4
    )
    async def next(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if self.messages and self.current < len(self.messages) - 1:
            ...
            self.UpdateButtons()
            await interaction.response.edit_message(
                content=self.messages[self.current], embed=None, view=self
            )
        elif self.embeds and self.current < len(self.embeds) - 1:
            self.current += 1
            self.UpdateButtons()
            await interaction.response.edit_message(
                content=None, embed=self.embeds[self.current], view=self
            )
