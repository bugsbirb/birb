...
class AForumCreaton(commands.Cog):
    ...
    async def on_created_thread(self, thread: discord.Thread):
        ...
        if 'Bugs' in applied_tags:
            ...
            await thread.send(content='<@&1092977919858593912>', embeds=(banner, embed), allowed_mentions=discord.AllowedMentions(roles=True))
            return
        else: 
         await thread.send(content='<@&1092977110412439583>', embeds=(banner, embed), allowed_mentions=discord.AllowedMentions(roles=True)) 
