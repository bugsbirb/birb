...
class webGuildJoins(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        if channel:
            ...
            await webhook.send(f"<:join:1140670830792159373> I am now in {len(self.client.guilds)} guilds.", username=guild.name, avatar_url=guild.icon, allow_mentions=False)
        owner = guild.owner
        view = Support()
